...
{var pinSpacing=(360/this.pins.number);for(i=1;i<=this.pins.number;i++)
{this.ctx.save();this.ctx.strokeStyle=this.pins.strokeStyle;this.ctx.lineWidth=this.pins.lineWidth;this.ctx.fillStyle=this.pins.fillStyle;this.ctx.translate(this.centerX,this.centerY);this.ctx.rotate(this.degToRad(i*pinSpacing+this.rotationAngle));this.ctx.translate(-this.centerX,-this.centerY);this.ctx.beginPath();this.ctx.arc(this.centerX,(this.centerY-this.outerRadius)+this.pins.outerRadius+this.pins.margin,this.pins.outerRadius,0,2*Math.PI);if(this.pins.fillStyle)
...
{character=lines[i].charAt(c);if(fillStyle)
...
{var centerAdjustment=0;if(lines[i].length>1)
{centerAdjustment=(yInc*(lines[i].length-1)/2);}
var yPos=(this.centerY+this.innerRadius+((this.outerRadius-this.innerRadius)/2))+centerAdjustment+margin;for(var c=(lines[i].length-1);c>=0;c--)
{character=lines[i].charAt(c);if(fillStyle)
this.ctx.fillText(character,this.centerX+lineOffset,yPos);if(strokeStyle)
this.ctx.strokeText(character,this.centerX+lineOffset,yPos);yPos-=yInc;}}
this.ctx.restore();}
...
{radius=this.outerRadius-margin;this.ctx.textBaseline='bottom';radius-=(fontSize*(lines.length-1));}
else if(alignment=='center')
{radius=this.innerRadius+margin+((this.outerRadius-this.innerRadius)/2);this.ctx.textBaseline='middle';}
var anglePerChar=0;var drawAngle=0;if(lines[i].length>1)
{this.ctx.textAlign='left';anglePerChar=(4*(fontSize/10));radiusPercent=(100/radius);anglePerChar=(anglePerChar*radiusPercent);totalArc=(anglePerChar*lines[i].length);drawAngle=seg.startAngle+(((seg.endAngle-seg.startAngle)/2)-(totalArc/2));}
else
{drawAngle=(seg.startAngle+((seg.endAngle-seg.startAngle)/2));this.ctx.textAlign='center';}
drawAngle+=this.rotationAngle;drawAngle-=180;for(c=lines[i].length;c>=0;c--)
{this.ctx.save();character=lines[i].charAt(c);this.ctx.translate(this.centerX,this.centerY);this.ctx.rotate(this.degToRad(drawAngle));this.ctx.translate(-this.centerX,-this.centerY);if(strokeStyle)
...
{var centerAdjustment=0;if(lines[i].length>1)
{centerAdjustment=(yInc*(lines[i].length-1)/2);}
var yPos=(this.centerY-this.innerRadius-((this.outerRadius-this.innerRadius)/2))-centerAdjustment-margin;for(var c=0;c<lines[i].length;c++)
{character=lines[i].charAt(c);if(fillStyle)
this.ctx.fillText(character,this.centerX+lineOffset,yPos);if(strokeStyle)
this.ctx.strokeText(character,this.centerX+lineOffset,yPos);yPos+=yInc;}}
this.ctx.restore();}
...
{var radius=0;if(alignment=='inner')
{radius=this.innerRadius+margin;this.ctx.textBaseline='bottom';radius+=(fontSize*(lines.length-1));}
else if(alignment=='outer')
...
{drawAngle=(seg.startAngle+((seg.endAngle-seg.startAngle)/2));this.ctx.textAlign='center';}
drawAngle+=this.rotationAngle;for(c=0;c<(lines[i].length);c++)
{this.ctx.save();character=lines[i].charAt(c);this.ctx.translate(this.centerX,this.centerY);this.ctx.rotate(this.degToRad(drawAngle));this.ctx.translate(-this.centerX,-this.centerY);if(strokeStyle)
this.ctx.strokeText(character,this.centerX,this.centerY-radius+lineOffset);if(fillStyle)
this.ctx.fillText(character,this.centerX,this.centerY-radius+lineOffset);drawAngle+=anglePerChar;this.ctx.restore();}}}
lineOffset+=fontSize;}}
...
{if(this.animation)
{this.computeAnimation();winwheelToDrawDuringAnimation=this;var properties=new Array(null);properties[this.animation.propertyName]=this.animation.propertyValue;properties['yoyo']=this.animation.yoyo;properties['repeat']=this.animation.repeat;properties['ease']=this.animation.easing;properties['onUpdate']=winwheelAnimationLoop;properties['onComplete']=winwheelStopAnimation;this.tween=TweenMax.to(this,this.animation.duration,properties);}}
Winwheel.prototype.stopAnimation=function(canCallback)
{if(winwheelToDrawDuringAnimation)
{winwheelToDrawDuringAnimation.tween.kill();winwheelStopAnimation(canCallback);}
...
{defaultOptions={'visible':true,'number':36,'outerRadius':3,'fillStyle':'grey','strokeStyle':'black','lineWidth':1,'margin':3,};for(var key in defaultOptions)
{if((options!=null)&&(typeof(options[key])!=='undefined'))
